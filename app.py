...
# ---------------------------------------------------------
# 수불부 전체 내역(집계 + 일별 수불부) 팝업 인쇄 컴포넌트
# ---------------------------------------------------------
def render_full_subul_print(df_summary, df_detail, period_str):
    ...
    <div style="width:100%;">
        <button onclick="
            ...
            pWin.document.write('<style>body{{font-family:sans-serif;padding:20px;color:#333;}} h2{{color:#1e3a8a;margin-bottom:5px;}} .period{{font-size:14px;color:#475569;margin-bottom:15px;font-weight:bold;}} .metric-table{{width:100%;margin-bottom:20px;border-spacing:8px;border-collapse:separate;}} .metric-card{{background:#f8fafc;border:1px solid #cbd5e1;border-radius:6px;padding:8px;text-align:center;}} .metric-title{{font-size:12px;color:#64748b;font-weight:bold;}} .metric-val{{font-size:15px;color:#0f172a;font-weight:bold;margin-top:2px;}} .print-table{{width:100%;border-collapse:collapse;margin-top:10px;font-size:11px;}} .print-table th,.print-table td{{border:1px solid #cbd5e1;padding:5px;text-align:center;}} .print-table th{{background-color:#f1f5f9;font-weight:bold;}}</style></head><body>');
            ...
            pWin.document.write('<h3 style=\"margin-top:25px;\">2. 일자별 개별 수불 상세 내역 (일별 수불부)</h3>');
