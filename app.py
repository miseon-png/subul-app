...
# ---------------------------------------------------------
# 2. 안전한 날짜 변환 전처리 함수 (핵심 추가)
# ---------------------------------------------------------
def safe_parse_date(series):
    """어떤 형태의 날짜 데이터가 들어와도 안전하게 python date 타입으로 변환"""
    # 1. pd.to_datetime으로 변환 (에러 발생 시 NaT 처리, 다양한 형식 지원)
...
with tab3:
    ...
    with col_date1:
        # 2024-07-01 등 과거 데이터 조회를 위해 시작일 기본값을 올해 1월 1일로 설정
        start_date = st.date_input("시작일", value=date(2024, 1, 1), key="tab3_start")
    with col_date2:
        end_date = st.date_input("종료일", value=datetime.today(), key="tab3_end")
    with col_vendor:
        selected_vendor = st.selectbox("입고 거래처 필터", ["전체"] + INBOUND_VENDORS, key="tab3_vendor")
...
# ---------------------------------------------------------
# TAB 4: 전체 내역 조회 & 기간 검색 및 다운로드 (날짜 처리 강화)
# ---------------------------------------------------------
with tab4:
    ...
    with ctrl1:
        s_date = st.date_input("조회 시작일", value=date(2024, 1, 1), key="tab4_sdate")
    with ctrl2:
    ...
    try:
        ...
        if data:
            ...
            # 안전한 날짜 파싱 적용
            df_all["날짜_검색용"] = safe_parse_date(df_all["날짜"])
            
            # 날짜 유효한 데이터 필터링
            df_filtered = df_all[
                (df_all["날짜_검색용"].notnull()) &
                (df_all["날짜_검색용"] >= s_date) & 
                (df_all["날짜_검색용"] <= e_date)
            ].copy()
            ...
            df_filtered = df_filtered.drop(columns=["날짜_검색용"]) # 임시 컬럼 제거
